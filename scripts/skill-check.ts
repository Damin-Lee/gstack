...
const CLAUDE_HOST_CONFIG = getHostConfig('claude');
...
function templateOutputSkillName(output: string): string {
  const dir = path.dirname(output);
  return dir === '.' ? '' : path.basename(dir);
}

function isSkippedForClaudeHost(output: string): boolean {
  const skillName = templateOutputSkillName(output);
  return skillName !== '' && (CLAUDE_HOST_CONFIG.generation.skipSkills ?? []).includes(skillName);
}
...
for (const { tmpl, output } of TEMPLATES) {
  ...
  if (!fs.existsSync(outPath)) {
    if (isSkippedForClaudeHost(output)) {
